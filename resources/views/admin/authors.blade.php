            ...
            <div class="row">
                ...
                <div class="card">
                    <div class="card-header d-flex">
                        <h3 class="card-title">
                        ALL USER LIST
                        <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip" data-placement="top" title="Total Number of User's" style="font-size: 18px; font-weight: 500;">{{ $authors->count() }}</span>
                        </h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                            ...
                            @foreach ($authors as $key=>$author)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <input data-id="{{$author->id}}" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-size="small" data-toggle="toggle" data-style="slow" data-on="Active" data-off="Deactive" {{ $author->status ? '' : 'checked' }}>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <button type="button" class="btn btn-sm btn-danger" onclick="deleteAuthors({{ $author->id }})"><i class="nav-icon fas fa-trash-alt" style="margin-right: 5px;"></i>Delete</button>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex">
                                <h3 class="card-title">
                                    ऑल यूजर लिस्ट
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip" data-placement="top" title="यूजर की कुल संख्या" style="font-size: 18px; font-weight: 500;">{{ $authors->count() }}</span>
                                </h3>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                    ...
                                    <tbody>
                                        @foreach ($authors as $key=>$author)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteAuthors({{ $author->id }})"><i class="nav-icon fas fa-trash-alt" style="margin-right: 5px;"></i>डिलीट</button>
                                                    <form id="delete-form-{{ $author->id }}" action="{{ route('admin.author.destroy', $author->id) }}" method="POST" style="display: none;">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" id="delAuthor">Yes, Delete</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="delModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    क्या आप वाकई इस ऑथर को हटाना चाहते हैं?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">नहीं, रद्द करें</button>
                    <button type="button" class="btn btn-danger" id="delAuthor">हाँ, हटाएं</button>
                </div>
            </div>
        </div>
    </div>
...
@push('js')
    ...
    <script>
        function deleteAuthors(id)
        {
            ...
            document.getElementById("delAuthor").addEventListener("click", function(){
