    ...
    @if (session('locale') == 'en')
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                    <h1 class="m-0">Latest News Page</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex">
                                <h3 class="card-title">
                                ALL NEWS LIST
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip" data-placement="top" title="Total Number of Newse's" style="font-size: 18px; font-weight: 500;">{{ $newses->count() }}</span>
                                </h3>
                                <a class="btn btn-success ml-auto" href="{{ route('user.news.create') }}"><i class="nav-icon fas fa-plus-circle" style="margin-right: 5px;"></i>ADD NEWS</a>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                ...
                                <tbody>
                                    @foreach ($newses as $key=>$news)
                                        <tr>
                                            ...
                                            <td>{!! Str::limit($news->discription, '150') !!}</td>
                                            <td>
                                                <img src="{{ asset('public/storage/news-english/'.$news->image) }}" alt="Feature-Image" width="200" height="140" class="rounded">
                                            </td>
                                            <td>{{ $news->created_at->format('d-M-Y') }}</td>
                                            <td class="text-center">
                                                {{-- <a href="#" class="btn btn-sm btn-success"><i class="nav-icon fas fa-eye"></i></a> --}}

                                                <a href="{{ route('user.news.edit', $news->id) }}" class="btn btn-sm btn-info"><i class="nav-icon fas fa-edit"></i></a>

                                                <button type="button" class="btn btn-sm btn-danger" onclick="deleteNews({{ $news->id }})"><i class="nav-icon fas fa-trash-alt"></i></button>
                                                <form id="delete-form-{{ $news->id }}" action="{{ route('user.news.destroy', $news->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </td>
    ...
    @if (session('locale') == 'hi')
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                    <h1 class="m-0">लेटेस्ट न्यूज़ पेज</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="#">मुख्य पेज</a></li>
                            <li class="breadcrumb-item active">लेटेस्ट न्यूज़</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex">
                                <h3 class="card-title">
                                    ऑल न्यूज़ लिस्ट
                                <span class="badge badge-info pt-1 pb-1 pl-2 pr-2 ml-1" data-toggle="tooltip" data-placement="top" title="न्यूज़ की कुल संख्या" style="font-size: 18px; font-weight: 500;">{{ $newses->count() }}</span>
                                </h3>
                                <a class="btn btn-success ml-auto" href="{{ route('user.news.create') }}"><i class="nav-icon fas fa-plus-circle" style="margin-right: 5px;"></i>नई खबर जोड़ें</a>
                            </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="tblPost" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    @foreach ($newses as $key=>$news)
                                        <tr>
                                            ...
                                            <td>{!! Str::limit($news->discription, '150') !!}</td>
                                            <td>
                                                <img src="{{ asset('public/storage/news-hindi/'.$news->image) }}" alt="Feature-Image" width="200" height="140" class="rounded">
                                            </td>
                                            <td>{{ $news->created_at->format('d-M-Y') }}</td>
                                            <td class="text-center">
                                                {{-- <a href="#" class="btn btn-sm btn-success"><i class="nav-icon fas fa-eye"></i></a> --}}
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" id="delNews">Yes, Delete</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="delModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    क्या आप वाकई इस न्यूज़ को हटाना चाहते हैं ?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">नहीं, रद्द करें</button>
                    <button type="button" class="btn btn-danger" id="delNews">हाँ, हटाएं</button>
                </div>
            </div>
        </div>
    </div>
...
@push('js')
    ...
    <script>
        ...
        function deleteNews(id)
        {
            ...
            document.getElementById("delNews").addEventListener("click", function(){
